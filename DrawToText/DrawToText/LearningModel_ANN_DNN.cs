...
namespace LearningModel//Written by Soomin Park, In ‎2022‎-07‎-09 saturday, 8:26:50 pm.
{
    ...
    class LearningModel<T> : Base.LearningModelBase<T>
    {
        ...
        public void SaveModelData()
        {
            ...
            sb.Append(layerModel.Length);
            string data = "";
            for (int i = 0; i < layerModel.Length; i++)
            {
                data += (i == layerModel.Length - 1) ? layerModel[i].ToString() : layerModel[i].ToString() + ",";
            }
            sb.Append(Environment.NewLine + data);
        ...
        public void LoadModelData()
        {
            ...
            Type t = Type.GetType(loaded[0]);
            object lm = Activator.CreateInstance(typeof(LearningModel<>).MakeGenericType(t));

            void SetProperty(object o, string name, object val)
            {
                PropertyInfo pi = o.GetType().GetProperty(name);
                pi.SetValue(o, Convert.ChangeType(val, pi.PropertyType), null);
            }*/
        }
        ...
        public void LoadModelData_Xml()
        {
            ...
            using (var reader = new StreamReader(Environment.CurrentDirectory + @"\ModelData.xml"))
            {
                ...
                activationFunctions = new Func<T, T>[layerModel.Length];
                activationPrimeFunctions = new Func<T, T>[layerModel.Length];
                Initialize_Silent();
