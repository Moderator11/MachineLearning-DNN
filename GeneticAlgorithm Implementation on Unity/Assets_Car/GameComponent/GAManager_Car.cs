...
public class GAManager_Car : MonoBehaviour
{
    ...
    void Start()
    {
        dummy = new LearningModel<float>(new int[] { 12, 10, 10, 10, 2 }, new string[] { "leakyrelu", "leakyrelu", "leakyrelu", "leakyrelu", "sigmoid" }, "MSE");
        dummy.Initialize_Silent();
        ga = new GeneticAlgorithm<float>(poolSize, dummy, 1, -1, 0.9f, 0.25f);
        tests = new CarController[poolSize];
        NewGeneration();
    }
    ...
    void NewGeneration()
    {
        ...
        for (int i = 0; i < poolSize; i++)
        {
            tests[i] = Instantiate(testInstance).GetComponent<CarController>();
            tests[i].agent = new LearningModel<float>(new int[] { 12, 10, 10, 10, 2 }, new string[] { "leakyrelu", "leakyrelu", "leakyrelu", "leakyrelu", "sigmoid" }, "MSE");
